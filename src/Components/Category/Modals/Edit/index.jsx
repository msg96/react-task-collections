...
export const EditCategoryModal = ({ data, exithandle }) => {
  ...
  async function UpdateFunc(e) {
    ...
    if (inputText === "") {
      handledMessage(
        "Defina um nome de pelomenos 1 caracter para sua categoria!."
      );
      setBusy(null);
      return;
    }
    if (inputText === data.data.name) {
      handledMessage("O novo nome da categoria não pode ser o antigo!.");
      setBusy(null);
      return;
    }

    const Checker = query(
      collection(DataBase, TableNames.categories),
    ...
    if (checkedDocks.empty) {
      const curDoc = doc(DataBase, TableNames.categories, data.id);
      const UpdatedDoc = {
        name: inputText,
      };
      ...
      handledMessage("Categoria já existente");
      setBusy(null);
    }
  }
